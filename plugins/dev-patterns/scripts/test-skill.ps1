...
$pluginDir = Join-Path $repoRoot 'plugins\dev-patterns'
$scenariosFile = Join-Path $pluginDir 'tests\scenarios.txt'

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$resultsDir = Join-Path $env:TEMP "aws-cdk-skill-test-$timestamp"
$workspace = Join-Path $env:TEMP "aws-cdk-skill-workspace-$timestamp"
...
$skillRoot = Join-Path $pluginDir 'skills\aws-cdk-patterns'
if (-not (Test-Path -LiteralPath $skillRoot -PathType Container)) {
    Write-Error "aws-cdk-patterns skill directory not found at $skillRoot"
    exit 1
}
