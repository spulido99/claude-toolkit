#!/usr/bin/env bash
# Test harness for expo-react-native skill retrieval.
#
...
# BASH_SOURCE[0] is .../plugins/dev-patterns/skills/expo-react-native/scripts/test-skill.sh
# Go up 3 levels to reach plugins/dev-patterns
PLUGIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SCENARIOS_FILE="$(dirname "${BASH_SOURCE[0]}")/../tests/scenarios.txt"

TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
RESULTS_DIR="/tmp/expo-react-native-skill-test-$TIMESTAMP"
WORKSPACE="/tmp/expo-react-native-skill-workspace-$TIMESTAMP"
...
if [[ ! -d "$PLUGIN_DIR/skills/expo-react-native" ]]; then
  echo "Error: expo-react-native skill directory not found at $PLUGIN_DIR/skills/expo-react-native" >&2
  exit 1
fi
