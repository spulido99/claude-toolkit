...
# PSScriptRoot = .../plugins/dev-patterns/skills/expo-react-native/scripts
...
$resultsDir = Join-Path $env:TEMP "expo-react-native-skill-test-$timestamp"
$workspace = Join-Path $env:TEMP "expo-react-native-skill-workspace-$timestamp"
...
$skillRoot = Join-Path $pluginDir 'skills\expo-react-native'
if (-not (Test-Path -LiteralPath $skillRoot -PathType Container)) {
    Write-Error "expo-react-native skill directory not found at $skillRoot"
    exit 1
}
