...
# BASH_SOURCE[0] is .../plugins/dev-patterns/skills/dynamodb-design/scripts/test-skill.sh
# Go up 3 levels to reach plugins/dev-patterns
PLUGIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
SCENARIOS_FILE="$(dirname "${BASH_SOURCE[0]}")/../tests/scenarios.txt"

TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
RESULTS_DIR="/tmp/dynamodb-design-skill-test-$TIMESTAMP"
WORKSPACE="/tmp/dynamodb-design-skill-workspace-$TIMESTAMP"
...
if [[ ! -d "$PLUGIN_DIR/skills/dynamodb-design" ]]; then
  echo "Error: dynamodb-design skill directory not found at $PLUGIN_DIR/skills/dynamodb-design" >&2
  exit 1
fi
